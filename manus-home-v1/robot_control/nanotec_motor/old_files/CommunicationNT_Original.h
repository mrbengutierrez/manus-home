 ...
 * Number of ticks (in encoder resolution through reductor) for 1 revolution.
 * Difference of 16^8-1 for a multiple of ONEREV.
 * Stiffness constant.
...
class CommunicationNT
{
  private:
    ...
    /**
     * Initial and last positions. Each 2000 is a revolution of 360 degrees.
     * Last angle, difference and torque.
     */
    double _initPos, _lastPos, _lastAng, _lastDif, _lastTor;

    /**
     * 1 Common case with small difference. Both values closer.
     * 2 When is going from a value near to zero to a value near to 16^8.
     * 3 When is going from a value near to 16^8 to a value near to zero.
     */
    int _changes;
  ...
  public:
    ...
    void torqueMode( int torque, int max, int maxCurr, int nomCurr, int slope );
    void changeTorque( int torque );
    double readPosition( int t );
    void stop();
    void closePort();
  ...
  private:
    ...
    void sendCommand( unsigned char *obj, int size, int data );
