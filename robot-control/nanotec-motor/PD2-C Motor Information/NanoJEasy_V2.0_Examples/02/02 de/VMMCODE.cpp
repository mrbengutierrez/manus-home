//in diesem Beispiel wird der Drehzahl-Modus gewählt, die state machine eingeschaltet und mit einem Eingang freigegeben
...
void user()
{
	...
	Out.OperationMode = 2;				// setzt den Operationsmodus auf Drehzahlmodus (mit Mapping, Zeile 5-8)
	//od_write(0x6060,0x00, 2);			// setzt ebenfalls den Operationsmodus (ohne Mapping, Zeile 5-8)
		
	Out.TargetVelocity = 200;				// setzt die Zielgeschwindigkeit auf 200 U/min (Standardeinheit)(mit Mapping, Zeile 5-8)
	//od_write(0x6042,0x00, 200);		// setzt die Zielgeschwindigkeit auf 200 U/min (Standardeinheit)(ohne Mapping, Zeile 5-8)
		
//3. Schritt: State maschine hochfahren, Eingang als "Freigabe" verwenden
