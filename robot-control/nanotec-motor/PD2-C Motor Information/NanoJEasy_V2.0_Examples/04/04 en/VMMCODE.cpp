...
void user()
{
	...
	InOut.TargetVelocity = 0;				// set the target velocity to 0 rpm (basicvalue)(with mapping, line 5-9)
...
//3. Step: switch on the state machine, use enable input, reading analog input
	...
	{
		InOut.TargetVelocity = In.AnalogInput;// target velocity=analogue value (0-1023) 
		...
		if(InOut.TargetVelocity>200)  	// if target velocity >200 then...
		{
			Out.Outputs = 0x10000;      // Output 1 high
		}
		else
		{
			Out.Outputs = 0x00000;      // else Output 1 low
			}	
