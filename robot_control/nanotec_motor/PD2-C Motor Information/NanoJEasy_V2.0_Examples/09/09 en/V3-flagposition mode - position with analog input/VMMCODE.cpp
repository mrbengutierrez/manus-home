// in this example the position mode will be selected and started, after the input 1 is triggered, the motor moves to a analog set position (flagposition mode)
...
//3. Step: switch on the state machine
	...
	{	
		...
		if((In.Inputs & 0x10000) == 0x10000)				// if input 1 (trigger)wenn Eingang 1 (trigger) high
			{
			Out.TargetPosition = In.ActualPosition + (In.AnalogInput * 10);	  //sets the new target position depending on the actual position and analog input
